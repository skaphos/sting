...
4. **Given** any completed query, **When** the result is returned, **Then** it reports the
   provider requests actually consumed and the quota remaining at completion.
...
- **Window starts before the repository's first commit**: the start boundary resolves to the
  repository's root state; the change set is everything added since inception, and the resolved
  boundary is reported as such.
...
- **FR-008**: The system MUST resolve the window's boundary commits deterministically from the
  window and reference, and MUST report both resolved boundary identifiers in the result.
...
- **FR-012**: Users MUST be able to set a ceiling on the provider requests a single query may
  consume.
...
- **FR-014**: Every result MUST report the provider requests consumed and the caller's remaining
  quota, including the quota reset time when the provider supplies one.
...
- **FR-022**: All new capability MUST be read-only. No path may create, update, or delete any
  provider-side object, and any new agent-facing tool MUST be annotated read-only and MUST derive
  its auto-approval status from the existing single source of truth.
...
- **FR-024**: Any change to the shape of the public result contract MUST bump the result schema
  version in the same change.
- **FR-025**: The capability MUST be available for GitHub. Where the GitLab equivalent differs in
  coverage or fidelity, the difference MUST be documented and disclosed in the result rather than
  papered over; where it is not yet implemented, requesting it MUST fail with a clear,
  provider-specific reason.
- **FR-026**: The capability MUST be reachable from both the local CLI and the agent-facing tool
  surface, with the structured output as the contract and any human-readable rendering as a view
  of it.
...
- **Resolved Boundaries**: The two commits that define the window's start and end state on the
  compared reference, plus how each was resolved and whether they share ancestry.
...
- **GitHub is the primary provider for this capability.** GitLab parity is required to be either
  delivered or explicitly documented as a coverage difference; it is not assumed automatic.
- **Existing query behavior is preserved.** Author-scoped queries keep working as they do today;
  this feature adds a repository-scoped question and adds cost controls to both. No existing
  default becomes more expensive.
...
- **Constitution I (Read-Only by Design)** → FR-022.
- **Constitution II (Evidence-Grade, Explainable Output)** → FR-014, FR-015, FR-017, FR-020.
- **Constitution III (Deterministic, Reconstructible Queries)** → FR-004, FR-008, FR-010, FR-019,
  FR-023.
...
- **ADR 0003 (Multi-runtime installer and read-only safety)** → any new agent-facing tool derives
  its auto-approval status from the existing read-only tool registry (FR-022).
